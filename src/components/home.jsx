    ...
    <Container sx={{ marginTop: 12, marginBottom: 8 }}>
      {load ? (
        ...
        <>
            ...
            {statistics.map((statistic, index) => {
              return (
                <Stack direction="column" key={index}>
                  ...
                  >{`Total ${statistic.type}`}</Typography>
                  <Typography variant="h6">
                    {numberformatting(Number(statistic.value))}
                  </Typography>
                </Stack>
              );
            })}
          ...
          >
            <div className="cards">
              {toppopularcoins.map((popularcoin, index) => {
                const { uuid, name, iconUrl, price, change, marketCap } =
                  popularcoin;
                return (
                  <CryptoCard
                    key={index}
          ...
          >
            <div className="cards">
              {Array.from(cryptonews?.slice(0, 4), (feed, index) => {
